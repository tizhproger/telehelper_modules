...
import base_loader, utils

logger = logging.getLogger(__name__)


@base_loader.tds
class YourMod(base_loader.Module):
    """Description for module"""  # Translateable due to @base_loader.tds
    strings = {"cfg_doc": "This is what is said, you can edit me with the configurator",
               "name": "A Name",
               "after_sleep": "We have finished sleeping!"}

    def __init__(self):
        self.config = base_loader.ModuleConfig("CONFIG_STRING", "hello", lambda m: self.strings("cfg_doc", m))

    @base_loader.unrestricted  # Security setting to change who can use the command (defaults to owner | sudo)
